...
input_text = "For important files, data, information, etc., transmitted and stored in a computer system, it is generally necessary to follow several best practices to ensure their security, integrity, and availability. Here are some key steps:Encryption: Encrypt files and data both in transit and at rest to protect them from unauthorized access. Use strong encryption algorithms and manage encryption keys securely. "
# print(input_text)
...
wm_precessor_message_model = WmProcessorMessageModel(message_model=lm_message_model,tokenizer=tokenizer,
    encode_ratio=5,max_confidence_lbd=0.5,strategy='max_confidence_updated', message=[102])
...
lm_message_model_update = LMMessageModel_update(tokenizer=tokenizer,lm_model=model,lm_tokenizer=tokenizer,
    delta = 1.5, lm_prefix_len=10, lm_topk=-1, message_code_len = 10,random_permutation_num=500)
...
output_tokens_update = model.generate(**tokenized_input, max_new_tokens=60, num_beams=4,#top_k=50,top_p=0.95,do_sample=True,#num_beams=4,
                               logits_processor=LogitsProcessorList(
                                   [min_length_processor, repetition_processor,
