...
def tokenize_for_generation(
    ...
    # print("Content of example:", example)
...
def collate_batch(input_ids: list, collator: DataCollatorWithPadding = None):
    ...
    input_ids = [torch.tensor(x) if not isinstance(x, torch.Tensor) else x for x in input_ids]

    # Check the shape of the tensors
    assert (
        input_ids[0].dim() == 2 and input_ids[0].shape[0] == 1 and input_ids[0].shape[1] > 0
...
def generate(
    ...
    # print("Type of examples['input_ids']:", type(examples["input_ids"]))
    # print("Content of examples['input_ids']:", examples["input_ids"])
