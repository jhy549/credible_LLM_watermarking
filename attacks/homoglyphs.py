...
class Homoglyphs:
    ...
    #             yield "".join(variant)
    def _get_combinations(self, text, ascii=False, attack_ratio=0.1):
        variations = []
        for char in text:
            # 根据攻击比例来决定是否将这个字符的同形异义字添加到variations列表中
            if random.random() < attack_ratio:
                ...
                # 如果这个字符不受攻击，就将它自己添加到variations列表中
                variations.append([char])

        if variations:
            # 从每个variations中随机选择一个元素，然后将结果连接成字符串并返回
            variant = [random.choice(variation) for variation in variations]
            yield "".join(variant)
